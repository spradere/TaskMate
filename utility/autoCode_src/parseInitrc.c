...
void parseInitrc(parse_init_rc_type_t TYPE, modules_database_t *data_base,
  ...
  while ((module_count < AUTOCODE_MODULE_COUNT_MAX) &&
    ...
    // procced args
    if ((tok.count > 0) &&
        (strcmp(tok.tokens[0], "#") != 0)) // skip empty line or comment
    {
      ...
      if (tok.count == 1) {
        data_base->run_level_module_count[TYPE][mod->status_default]++;
      }

      // parse commands
      for (int i = 1; i < tok.count; i++) {
        int err =
            initrcCmdDispatch(tok.tokens[i], &mod->modules[module_count].status,
