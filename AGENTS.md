...
- Update the relevant tests for every code change unless the prompt explicitly
  says not to update tests. Run the affected tests and report any failure.
...
`conf/arch_valid_matrix.md` is the conceptual map of the current architecture.
Only `interfaces` is deliberately cross-cutting. Read the main stack from top to
...
- `srcs/user/tasks/` contains the application threads. Tasks use service,
  ...
  drivers directly. Their records, stacks, and initial contexts are
...
- `srcs/hal/` is the target specific code implementation for real or 
  virtual drivers, architecture setup, generic reusable drivers `hal/drivers`, 
  board or host specifications and makefiles.
...
```sh
bmake TARGET=<target>       # select a target
...
Do not review audit files; architecture notes nor documentation overall.
