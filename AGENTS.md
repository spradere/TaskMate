...
- `srcs/system/services/` contains reusable system threads above the kernel. The
  current message and SCLI services use syscalls for thread cooperation and a
  temporary lateral HAL bridge for USART/LCD I/O. This bridge is scheduled for
  removal and must not be extended or used as a precedent.
...
The normal application path is therefore tasks -> services or `sysCall` ->
`sysCore` -> `HAL Public` -> target implementation -> hardware. The lateral
service-to-HAL bridge is a temporary migration path outside this intended flow
and must ultimately disappear. By contrast, `interfaces` and `tm_libc` are
permanent transversal layers by architectural choice; their cross-cutting role
does not relax their documented APIs or build-enforced access rules.
