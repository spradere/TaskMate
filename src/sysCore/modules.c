...
void mod_driversAlloc(void)
{
	...
	*(mod) = (mod_driver_item_t){.name = driver0_name,
								 .status = 1,
	...
	*(mod) = (mod_driver_item_t){.name = driver1_name,
								 .status = 1,
	...
	*(mod) = (mod_driver_item_t){
		.name = driver2_name, .status = 1, .init = hal_i2cInit, .start = hal_i2cStart, .stop = hal_i2cStop};

	mod = mod_driverGetPointer(3);
	const char *driver3_name = "hal_usart";
	*(mod) = (mod_driver_item_t){.name = driver3_name,
								 .status = 1,
								 .init = hal_usartInit,
	...
	*(mod) = (mod_driver_item_t){
		.name = driver4_name, .status = 2, .init = hal_lcdInit, .start = hal_lcdStart, .stop = hal_lcdStop};

	mod = mod_driverGetPointer(5);
	const char *driver5_name = "hal_ZS_042";
	*(mod) = (mod_driver_item_t){.name = driver5_name,
								 .status = 2,
								 .init = hal_ZS_042Init,
