...
static struct
{
	mod_driver_item_t drivers[MODULES_DRIVER_COUNT];
	mod_thread_item_t threads[MODULES_THREAD_COUNT];
	uint8_t thread_current;
} modules_db;

void mod_threadSetCurrent(uint8_t n) { modules_db.thread_current = n; }
uint8_t mod_threadGetCurrent(void) { return modules_db.thread_current; }

void mod_threadSetSTC(uint16_t count)
{
	modules_db.threads[modules_db.thread_current].software_time_counter = count;
}

uint16_t mod_threadGetSTC(void)
{
	return modules_db.threads[modules_db.thread_current].software_time_counter;
}

mod_driver_item_t *mod_driverGetPointer(uint8_t id) { return &modules_db.drivers[id]; }
mod_thread_item_t *mod_threadGetPointer(uint8_t id) { return &modules_db.threads[id]; }
