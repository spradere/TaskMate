...
void initThreads(void)
{
	...
	hal_threadContextInit(scli, &modules.threads[0].stack_pointer,
						  &modules.threads[0].stack[THREAD_STACK_SIZE - 1]);
	...
	hal_threadContextInit(msg, &modules.threads[1].stack_pointer,
						  &modules.threads[1].stack[THREAD_STACK_SIZE - 1]);
	...
	hal_threadContextInit(task1, &modules.threads[2].stack_pointer,
						  &modules.threads[2].stack[THREAD_STACK_SIZE - 1]);
	...
	hal_threadContextInit(task2, &modules.threads[3].stack_pointer,
						  &modules.threads[3].stack[THREAD_STACK_SIZE - 1]);
...
void initDrivers(void)
{
	...
	modules.drivers[0] = (module_item_driver_t){.name = (uint8_t *)driver0_name,
												.status = 1,
												.init = hal_timerSchedulerInit,
												.start = hal_timerSchedulerStart,
												.stop = hal_timerSchedulerStop};
	const char *driver1_name = "hal_timerRTC";
	modules.drivers[1] = (module_item_driver_t){.name = (uint8_t *)driver1_name,
												.status = 1,
												.init = hal_timerRTCInit,
												.start = hal_timerRTCStart,
												.stop = hal_timerRTCStop};
	const char *driver2_name = "hal_i2c";
	modules.drivers[2] = (module_item_driver_t){.name = (uint8_t *)driver2_name,
												.status = 1,
												.init = hal_i2cInit,
												.start = hal_i2cStart,
												.stop = hal_i2cStop};
	const char *driver3_name = "hal_usart";
	modules.drivers[3] = (module_item_driver_t){.name = (uint8_t *)driver3_name,
												.status = 1,
												.init = hal_usartInit,
												.start = hal_usartStart,
												.stop = hal_usartStop};
	const char *driver4_name = "hal_lcd";
	modules.drivers[4] = (module_item_driver_t){.name = (uint8_t *)driver4_name,
												.status = 2,
												.init = hal_lcdInit,
												.start = hal_lcdStart,
												.stop = hal_lcdStop};
